...
const Settings: NextPage = () => {
  ...
  React.useEffect(() => {if(userData) user({variables: { userID: userData._id }})}, [userData])
  React.useEffect(() => {
    if(data) {
  ...
  React.useEffect(() => {
    ...
    if (updateError) {
      enqueueSnackbar({ message: 'Щось пішло нетак', key: `${new Date().getTime()}+${Math.random()}` })
      linearProgress(false)
    }
    if (updateData) {
      setData(updateData.updateUser)
      localStorage.setItem('userData', JSON.stringify({ ...updateData.updateUser }))
      enqueueSnackbar({ message: 'Інформація оновленна', key: `${new Date().getTime()}+${Math.random()}` })
    }
