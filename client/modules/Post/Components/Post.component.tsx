...
export const PostComponent: React.FC<IPropsComponent> = ({ post, useDate, refm, steps, style, colors, handleClickToUser }) => {
  ...
  return <Container >
    <Stack marginTop={10} flexDirection='row' alignItems='center' justifyContent='space-between'>
      ...
      <Typography variant='body2' sx={{ borderRight: `3px solid ${colors.darkGray}`, padding: '4px 6px 4px 0' }}>
    ...
    <Grid container marginTop={2}>
      ...
      <Grid item xs={8} marginTop={3}>
        ...
        {
          link &&
          <Link href={link} target='_blank' underline="none" color={colors.darkGray} display='flex' sx={{ alignItems: 'center', transition: '300ms', ':hover': { color: '#ed2945' } }}>
        ...
        <Stack flexDirection='row' width='100%' marginTop={2} sx={{ alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <IconButton aria-label="share" sx={{ marginLeft: 'auto', color: colors.icon }}>
            <Share />
          </IconButton>
        </Stack>
        <Stack flexDirection='row' marginTop={2}>
          <Stack flexDirection='row' sx={{ alignItems: 'center' }}>
            <Button onClick={() => handleClickToUser(author._id)} sx={{ padding: 0, borderRadius: '50%' }}>
              <UserAvatar size={70} name={author.name} avatar={author.avatar} />
            </Button>
            <Stack alignItems='center' marginLeft={2}>
              <Link onClick={() => handleClickToUser(author._id)} underline="none" marginBottom={0.7} variant="h6" sx={{ color: colors.darkGray, transition: '300ms', ':hover': { color: '#db4454' } }}>{author.name}</Link>
