...
export const Comment: React.FC<Props> = ({ item, isAnswerToComment, refetch }) => {
  ...
  return <Stack direction='row' marginTop={2} marginBottom={isAnswerToComment ? 1 : 5}>
    <UserAvatar size={60} avatar={author.avatar} name={author.name} />
    <Box width='100%' sx={{ paddingLeft: '30px' }}>
      ...
      {
        !isAnswerToComment && <Button onClick={handleClickOpenAnswer} sx={{ fontSize: '11px', textTransform: 'uppercase', color: 'rgba(0,0,0,.6)', transition: '300ms', ':hover': { color: '#db4454' } }}>
          відповісти
        </Button>
      }
