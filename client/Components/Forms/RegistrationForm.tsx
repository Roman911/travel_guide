...
export const RegistrationForm: React.FC<Props> = ({ config, handleClickShowPassword }) => {
  ...
  return <>
    ...
    <Controller
      ...
      render={({ field }) => <FormControl
        ...
        variant="standard"
      >
        <InputLabel htmlFor={types.PASSWORD_CONFIRMATION}>Повторіть пароль</InputLabel>
        <Input
          type={config.showPassword ? 'text' : types.PASSWORD}
          endAdornment={
            <InputAdornment position="end">
    ...
    />
    <Button type='submit' variant="contained" sx={{ width: '180px', marginTop: '20px' }} color='secondary' disabled={config.isDisabled}>Зареєструватися</Button>
  </>
}